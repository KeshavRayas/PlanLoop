...
function ask(
  args: string[],
  message: string,
): Promise<{ chunks: string[]; rawLen: number }> {
  return new Promise((resolve) => {
    ...
    c.on("error", (e) =>
      resolve({ chunks: [`SPAWN_ERR: ${String(e)}`], rawLen: 0 }),
    );
  });
}
...
async function main() {
  ...
  const padding = Array.from(
    { length: 60 },
    (_, i) =>
      `Posting paragraph ${i}: we build distributed systems with Go, Postgres, and Kubernetes.`,
  ).join("\n");
  const msg = [
  ...
  const r = await ask(
    ["run", "--format", "json", "--agent", "json-answer"],
    msg,
  );
