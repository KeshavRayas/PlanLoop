...
describe("classifyJob", () => {
  ...
  it("accepts Full Stack Developer with skill keywords", () => {
    const result = classifyJob(makeJob({
      title: "Developer",
  ...
  it("increases confidence with more skills", () => {
    const low = classifyJob(makeJob({
      title: "Junior Developer",
      description: "Python",
    }));
    const high = classifyJob(makeJob({
      title: "Junior Developer",
      description: "Python, React, AWS, Docker, Kubernetes, PostgreSQL, Redis, Kafka, GraphQL, TypeScript, Node.js, Git, CI/CD, Terraform",
    }));
    expect(high.confidence).toBeGreaterThanOrEqual(low.confidence);
    expect(high.confidence).toBeGreaterThan(0.80);
  });
...
describe("isEntryLevel", () => {
  ...
  it("accepts Graduate Engineer", () => {
    const result = isEntryLevel(makeJob({ title: "Graduate Software Engineer" }));
    expect(result.accepted).toBe(true);
  });
  ...
  it("accepts Associate Engineer", () => {
    const result = isEntryLevel(makeJob({ title: "Associate Software Engineer" }));
    expect(result.accepted).toBe(true);
  });
  ...
  it("accepts Fresher", () => {
    const result = isEntryLevel(makeJob({ title: "Fresher Software Engineer" }));
    expect(result.accepted).toBe(true);
  });

  it("accepts entry from API experience field", () => {
    const result = isEntryLevel(makeJob({
