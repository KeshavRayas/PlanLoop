...
function checkIds(
  ...
  if (!ids || ids.length === 0) {
    if (!allowEmpty) issues.push({ type: "EMPTY_SOURCES", text: `${owner}: no evidence cited` });
    return;
  }
  const seen = new Set<string>();
  for (const src of ids) {
    if (seen.has(src)) {
      issues.push({ type: "DUPLICATE_EVIDENCE", text: `${owner}: duplicate evidence id: ${src}` });
    }
    seen.add(src);
    if (!evidence.has(src)) {
      issues.push({ type: "UNKNOWN_SOURCE", text: `${owner} cites unknown evidence: ${src}` });
    }
  }
}
...
export function validateInterviewPrep(
  ...
  const allQuestions = [...output.technical, ...output.resumeBased, ...output.behavioral];
  for (const q of allQuestions) {
    checkIds(`question "${q.question.slice(0, 60)}"`, q.evidenceIds, evidence, issues, {
