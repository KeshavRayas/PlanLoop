...
type SchemaLike<T> = {
  safeParse: (raw: unknown) => { success: true; data: T } | { success: false; error: ErrorLike };
};
...
export function buildValidationRetryPrompt(prompt: string, issues: string): string {
  return (
...
export async function parseWithSingleRetry<T>(opts: {
  ...
  console.error(`[${opts.logTag}] schema invalid for job ${opts.jobId}: ${issues}`);
