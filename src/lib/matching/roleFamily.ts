...
export function classifyRoleFamily(title: string): RoleFamily {
  ...
  if (has(t, "forward deployed", "forward-deployed", " fde ", "(fde)"))
    return "FORWARD_DEPLOYED";
  ...
  if (has(t, "full stack", "fullstack", "full-stack", "mern", "mean"))
    return "FULL_STACK";
  if (
    has(
      t,
...
export function roleFit(
  ...
  if (family === "DATA_ANNOTATION")
    return { fit: "VETO", score: ROLE_FIT_SCORE.VETO };
  const vetoed = (goals.vetoed ?? []).map((s) => s.toUpperCase());
  const preferred = (goals.preferred ?? []).map((s) => s.toUpperCase());
  if (vetoed.includes(family))
    return { fit: "VETO", score: ROLE_FIT_SCORE.VETO };
  if (preferred.length === 0)
    return { fit: "ACCEPTABLE_FIT", score: ROLE_FIT_SCORE.ACCEPTABLE_FIT };
  if (preferred.includes(family))
    return { fit: "STRONG_FIT", score: ROLE_FIT_SCORE.STRONG_FIT };
  if (family === "OTHER")
    return { fit: "WEAK_FIT", score: ROLE_FIT_SCORE.WEAK_FIT };
  return { fit: "ACCEPTABLE_FIT", score: ROLE_FIT_SCORE.ACCEPTABLE_FIT };
}
